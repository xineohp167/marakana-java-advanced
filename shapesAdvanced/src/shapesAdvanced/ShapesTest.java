...
public class ShapesTest {
	...
	public void equalRectanglesMustBeEqual() {
		for (int i = 0; i < 100; i++) {
			Rectangle r1 = arbitraryRectangle(), r2 = new Rectangle(
	...
	public void equalSquaresMustBeEqual() {
		for (int i = 0; i < 100; i++) {
			Square s1 = arbitrarySquare(), s2 = new Square(s1.getWidth());
			assertEquals(s2, s1);
		}
	}
	...
	public void setRectangleWidthShouldChangeOnlyWidth() {
		for (int i = 0; i < 100; i++) {
			...
			int width = RANDOM.nextInt(50), height = r.getHeight();
			r.setWidth(width);
